...
class Solution
{
   ...
   #define append push_back
    ...
    void dfs(Node* parent, Node* vertex, vector<Node*> &v){
        mp[vertex]=parent;
        if (vertex->left!=nullptr) dfs(vertex, vertex->left, v);
        if (vertex->right!=nullptr) dfs(vertex, vertex->right, v);
    ...
    {
        vector<Node*> v;
        dfs(nullptr, root, v); int cnt=0;
        unordered_set<Node*> s;
        for(auto val: v){
            int i=k;
            auto node = val;
            while(node!=nullptr and i!=0){
                node = mp[node];
                i--;
            } 
            if (i==0 and node!=nullptr) {
                s.insert(node);
            }
        }
        return s.size();
